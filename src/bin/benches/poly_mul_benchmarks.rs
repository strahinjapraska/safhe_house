...
use safhe_house::math::ring::{mul, mul_no_mod, uniform_random_element, PolyMulAlgorithm};
...
fn run_benchmarks_write_to_file() -> Result<(), Box<dyn Error>>{
    ...
    for p in params.iter().take(params.len() -1){
        ...
        let _ = mul_no_mod(&a, &b, param.n(), PolyMulAlgorithm::Karatsuba, param.prec());
        let elapsed1 = time.elapsed(); 
        println!("Karatsuba time: {:?}", elapsed1);

        let time = Instant::now(); 
        let _= mul_no_mod(&a, &b, param.n(), PolyMulAlgorithm::Fft, param.prec());
        ...
        if param.n() < 30000{
            let time = Instant::now();
            let _ = mul_no_mod(&a, &b, param.n(), PolyMulAlgorithm::SchoolBook, param.prec());
        ...
        let time = Instant::now();
        let _ = mul(&a, &b, &param.p(), &param.w(), &param.w_inv(), &param.phi(), &param.phi_inv());
        let elapsed4 = time.elapsed();
        println!("NTT time: {:?}", elapsed4);


        println!("-----------------------------------------------------------------------");

        writer.write_record(&[elapsed1.as_millis().to_string(), elapsed2.as_millis().to_string(), elapsed3.as_millis().to_string(), elapsed4.as_millis().to_string(), param.n().to_string()])?;  
        let _= writer.flush();   
